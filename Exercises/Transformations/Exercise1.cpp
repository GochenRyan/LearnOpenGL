// 先旋转，后位移：实际上是先乘位移矩阵进行位移变换，再乘旋转矩阵进行旋转变换，表现为绕着原来的中心旋转。
...
void setTextureVisibility(GLFWwindow* window, Shader &shader, float &visibility);
...
int main() {
	...
	while (!glfwWindowShouldClose(window)) {
		...
		setTextureVisibility(window, ourShader, visibility);
